...
rs.initiate({
  _id: "shard1",
  members: [ { _id: 0, host: "shard1:27018" }, { _id: 1, host: "replica1:27021" } ]
});
let i=0;
while (true) {
  const s = rs.status();
...
rs.initiate({
  _id: "shard2",
  members: [ { _id: 0, host: "shard2:27019" }, { _id: 1, host: "replica2:27022" } ]
});
let i=0;
while (true) {
  const s = rs.status();
...
echo "[init] Dropping any stray somedb on replica1 (avoid 'local database exists' conflicts)..."
docker compose exec -T replica1 mongosh --port 27021 --quiet <<'EOF'
use somedb
db.dropDatabase()
print("[ok] dropped somedb on replica1 (if existed)")
EOF
...
echo "[init] Dropping any stray somedb on replica2 (avoid 'local database exists' conflicts)..."
docker compose exec -T replica2 mongosh --port 27022 --quiet <<'EOF'
use somedb
db.dropDatabase()
print("[ok] dropped somedb on replica2 (if existed)")
EOF
